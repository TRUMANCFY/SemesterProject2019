function [Un, Vn, Wn, hist, hist_con] = ADMM_order(X, nIter, alpha,mu, rho, U, V, W)
...
function [V_update] = update_V(X,U,V,W,Z,mu_V,rho)
    R = size(U,2);
    V_update = (bigX(X,V,U,W,2) + mu_V * eye(R)) / (bigGamma(U,W,rho) + mu_V * eye(R));
end
