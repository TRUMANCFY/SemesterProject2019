function [Un, Vn, Wn, hist] = LALM(X, nIter, alpha,mu, rho, U, V, W)
...
% s.t. U * 1 = 1 (the sum along with the row)
    ...
    fprintf('The initial loss before lagm is %f \n', norm(X - reconstruct(U,V,W)));
    
    fprintf('The contrain loss before lagm is %f \n', norm(sum(U,2) - ones(I,1)));
...
%        V_new = update_U(X,)
    ...
    fprintf('The initial loss after lagm is %f \n', norm(X - reconstruct(U,V,W)));
    fprintf('The contrain loss after lagm is %f \n', norm(sum(U,2) - ones(I,1)));
